...
# por defecto top_db = 40 
def trim(wav, top_db=40, min_silence_sec=0.8, min_sample_sec=None):
...
#                               top_db=top_db)[0, 1]
    endpoints = librosa.effects.split(wav, frame_length=frame_length,
                               hop_length=hop_length,
                               top_db=top_db)[:, 1]
    if min_sample_sec == None:
        return wav[:endpoints[0]]
    else:
        for ep in endpoints:
            if ep > int(min_sample_sec*hp.sr):
                return wav[:ep]
    #print shape
    #return wav[:endpoint]
    return wav

def spectrogram2wav(mag,min_sample_sec=None):
    ...
    wav = trim(wav,min_sample_sec=min_sample_sec)
...
def plot_test_alignment(alignment, file_index):
    """Plots the alignment
    alignments: A list of (numpy) matrix of shape (encoder_steps, decoder_steps)
    ...
    # cbar_ax = fig.add_axes([0.85, 0.15, 0.05, 0.7])
    fig.colorbar(im)
    plt.title('{} '.format(file_index))
    plt.savefig('{}/samples/{}.png'.format(hp.lang, file_index), format='png')
